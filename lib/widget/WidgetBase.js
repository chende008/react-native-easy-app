...
import FlattenStyle, { ResetStyle, SplitStyle, SplitTextProps, XSize, } from '../utils/StyleUtils';
...
export function XImage({ icon, iconSize, onPress, onLongPress, raw, style, ...props }) {
  const allStyle = raw ? style : ResetStyle(style) || { width: XSize(20), height: XSize(20) };// reset style
  const { borderRadius, borderWidth, borderColor, tintColor, ...otherStyles } = selfOr(allStyle, {});
  const newSize = raw ? iconSize : XSize(iconSize);
  const imgStyle = {
    width: (newSize || allStyle.width),
    height: (newSize || allStyle.height),
    borderColor,
    borderRadius,
    borderWidth,
    tintColor
  ...
  if (isFunc(onPress) || isFunc(onLongPress)) {
    ...
    return <XView onPress={onPress} style={[{ justifyContent: 'center', alignItems: 'center' }, otherStyles]}
...
export function XText(props) {
  let { text, icon, iconSize, iconMargin = 2, iconPosition = 'top', onPress, onLongPress, textExtend = false, raw, style = {}, resizeMode, iconStyle = {}, ...others } = props;
  ...
  if (isEmpty(icon)) {// If the Icon has no value, it is considered a plain text display
    if (isFunc(onPress) || isFunc(onLongPress)) {
      ...
      if (style.borderRadius > 0) {// Text for IOS does not support rounded corners
        return <View style={raw ? externalPure : external} {...viewProps}><Text
          style={internal}{...textProps}>{text}</Text></View>;
      }
      if (raw) {// Returns the original text component
        ...
        return <Text onPress={onPress} onLongPress={onLongPress}
                     style={FlattenStyle([external, internal])}{...others}>{text}</Text>;
      }
    }
  } else {
    ...
    const imageView = !isEmpty(icon) &&
      <XImage key={1} style={{ width: iconSize || 20, height: iconSize || 20, ...iconStyle }}
              resizeMode={selfOr(resizeMode, 'contain')} icon={icon}/>;
    ...
    if (isFunc(onPress)) {
      return <XView style={raw ? externalPure : external} onPress={onPress}
                    onLongPress={onLongPress} {...viewProps}>{widgets}</XView>;
    } else {
