...
export function XImage({icon, iconSize, onPress, onLongPress, raw, style, ...props}) {
    ...
    const imgStyle = {width: (newSize || allStyle.width), height: (newSize || allStyle.height), borderColor, borderRadius, borderWidth, tintColor};
    if (isFunc(onPress) || isFunc(onLongPress)) {
...
export function XText(props) {
    ...
    if (isEmpty(icon)) {// If the Icon has no value, it is considered a plain text display
        ...
        const imageView = !isEmpty(icon) && <XImage key={1} style={{width: iconSize || 20, height: iconSize || 20, ...iconStyle}} resizeMode={selfOr(resizeMode, 'contain')} icon={icon}/>;
