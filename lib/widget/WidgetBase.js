...
export function RFText(props) {
    let {text, icon, iconSize, iconMargin = 2, position = 'top', onPress, onLongPress, style = {}, textExtend = false, raw = false, ...others} = props;
    ...
    if (isEmpty(icon)) {// 若Icon没有值，则认为是纯文本展示
        ...
        let prePos = mainAxis.has(position);//图标前置(主轴方向前置)
        let horizontal = horizontalDirection.has(position);//图标、文本排列为水平方向
