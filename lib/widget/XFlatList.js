...
export default class XFlatList extends PureComponent {
    ...
    render() {
        ...
        const {onRefresh, onLoadMore, indicatorOffset, renderEmptyViewFunc, renderPreEmptyViewFunc, renderRooterViewFunc, indicatorProps, refreshStatus, ...props} = this.props;
        ...
        return <ViewTag refreshing={status === RefreshStatus.RefreshingData}
                        ...
                        ListEmptyComponent={() => this.renderEmptyView(status, isEmptyList, renderEmptyViewFunc, renderPreEmptyViewFunc)}
    ...
    renderEmptyView = (status, isEmpty, renderEmptyViewFunc, renderPreEmptyViewFunc) => {
        if (!this.hasLaunch) {
            if (renderPreEmptyViewFunc) return renderPreEmptyViewFunc()
            return <View style={{width: width, height: height * 0.7, justifyContent: 'center'}}>
                <XText style={{color: '#919BA5', fontSize: 14, alignSelf: 'center'}} text={RefreshStatus.PreData.text}/>
            </View>
        }
    ...
    static propTypes = {
        ...
        renderPreEmptyViewFunc: PropTypes.func,
    };
}
