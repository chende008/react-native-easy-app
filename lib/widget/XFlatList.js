...
export default class XFlatList extends PureComponent {
    ...
    render() {
        ...
        let {onRefresh, onLoadMore, indicatorOffset, emptyViewHeight,renderFooter,indicatorProps, ...props} = this.props;
        ...
        return <ViewTag refreshing={status === XFlatList.RefreshStatus.RefreshingData}
                        ...
                        refreshControl={<RefreshControl onRefresh={() => onRefresh()} refreshing={status === XFlatList.RefreshStatus.RefreshingData} progressViewOffset={selfOr(indicatorOffset, 50)} {...indicatorProps}/>}
                        keyExtractor={(item, index) => index.toString()}
                        ListEmptyComponent={() => this.renderEmptyView(status, isEmpty(forFlatList ? props.data : props.sections), emptyViewHeight)}
                        ListFooterComponent={() => renderFooter || this.renderFooter(status, isEmpty(forFlatList ? props.data : props.sections))}
