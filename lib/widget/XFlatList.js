...
export default class XFlatList extends PureComponent {
    ...
    render() {
        ...
        const {onRefresh, onLoadMore, indicatorOffset, renderEmptyViewFunc, renderRooterViewFunc, ...props} = this.props;

        const canLoadMore = status === RefreshStatus.Idle || status === RefreshStatus.NoData;
        ...
        const RefreshView = <RefreshControl
            ...
            progressViewOffset={selfOr(indicatorOffset, 50)}/>;
    ...
    renderEmptyView = (status, isEmpty, renderEmptyViewFunc) => {
        ...
        const currentText = (RefreshStatus.RefreshingData === status || !isEmpty) ? status.text : RefreshStatus.NoData.text;
        return <View style={{width: width, height: height * 0.7, justifyContent: 'center'}}>
            <XText style={{color: '#919BA5', fontSize: 14, alignSelf: 'center'}} text={currentText}/>
        </View>
    };
