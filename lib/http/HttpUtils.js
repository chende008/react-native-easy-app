...
export function getParams(data) {// Generate the generic s parameter
    let {headerSetFunc, paramSetFunc, baseUrl, finalParam = {}} = RFHttpConfig.default;
    ...
    paramSetFunc && paramSetFunc(finalParam, data);
    finalParam = {...finalParam, ...param};
    RFPackage(finalParam, paramArray);
    ...
    switch (finalContentType) {
        ...
        case RFApiConst.CONTENT_TYPE_FORM_DATA:
            result.params.body = RFPackage(finalParam, new FormData());
            break;
        case RFApiConst.CONTENT_TYPE_URLENCODED:
            result.params.body = paramArray.join('&');
            break;
    }
    if (!finalContentType.includes('charset')) {// set charset to utf-8
