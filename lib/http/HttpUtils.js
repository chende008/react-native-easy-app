...
export function getParams(data) {// Generate the generic s parameter
    ...
    result.params.headers = {ContenTypeStr: contentType, ...header};
    ...
    const finalType = result.params.headers[ContentTypeStr];
    switch (finalType) {
    ...
    if (!finalType.includes('charset')) {// set charset to utf-8
        result.params.headers[ContentTypeStr] = finalType + ';charset=utf-8'
    }
