...
export function getParams(data) {// Generate the generic s parameter
    let {headerSetFunc, paramSetFunc, baseUrl, commonParam = {}, globalEncodeComponent, contentType: configContentType} = HttpConfig[data.serverTag];
    ...
    let finalParam = encodeParams(commonParam, param, globalEncodeComponent, encodeComponent);
...
function encodeParams(commonParam, params, globalEncodeComponent, encodeComponent) {
    let finalEncode = globalEncodeComponent;
    if (encodeComponent != null) {
        finalEncode = encodeComponent;
    }
    if (finalEncode) {// need encode
        if (Array.isArray(params)) {
            params.map(item => encodeURLCompoment(item))
        } else {
            params = encodeURLCompoment({...commonParam, ...params})
        }
    } else {
...
function encodeURLCompoment(params) { // encodeURIComponent for params
