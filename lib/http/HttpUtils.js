...
export function getParams(data) {// Generate the generic s parameter
    let {headerSetFunc, paramSetFunc, baseUrl, finalParam = {}, contentType: configContentType} = HttpConfig[data.serverTag];
    let newUrl, paramArray = [], result = {...data};
    let {url, param = {}, method, contentType, header = {}, encodeURI, enableHeaderSetFunc, enableParamSetFunc} = data;
    ...
    XPackage(finalParam, paramArray, encodeURI);
    ...
    switch (finalContentType) {
        ...
        case XHttpConst.CONTENT_TYPE_FORM_DATA:
            result.params.body = XPackage(finalParam, new FormData(), encodeURI);
            break;
...
export function XPackage(origin, target, encodeURI) {
