...
export default class HttpRequest {
    ...
    constructor() {
        this.data = {
            ...
            disableHeaderSetFunc: false, // disable common headers set info
            disableParamSetFunc: false,  // disable common params set info
    ...
    disableHeaderSetFunc() {
        this.data.ignoreHeaderSetFunc = true;
        return this;
    }

    disableParamSetFunc() {
        this.data.disableParamSetFunc = true;
        return this;
    }
...
function exec(data, callback) {
    let {baseUrl, response = null, globalLoadingFunc} = RFHttpConfig;
...
function getContentType() {// Get the current configuration of contentType
    return selfOr(RFHttpConfig.contentType, RFHttpConst.CONTENT_TYPE_JSON);
}

function getTimeout() {
    let {timeout} = RFHttpConfig;
