...
export function RFSize(size): number {
    ...
    if (validReferSize()) {
        return Math.ceil(size * screenWidth / RFWidget.default.targetWidth);
    } else {
        return size;
    }
}
...
export function RTSize(size): number {
    ...
    if (validReferSize()) {
        const scale = Math.min(screenWidth / RFWidget.default.targetWidth, screenHeight / RFWidget.default.targetHeight);
...
export function validReferSize() {
    let {targetHeight, targetWidth, pixelRatio} = RFWidget.default;
